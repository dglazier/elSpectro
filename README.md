...
`elSpectro` is a C++ framework designed for the simulation of particle physics events, with a focus on electron scattering experiments. It provides a set of core libraries for defining particles, decays, kinematics, and event generation logic.
...
- **`core` Library (`libelSpectro`)**: This is the heart of the framework. It is a shared library that contains all the fundamental physics classes and logic for defining particles, managing decays, and generating events.
...
This section describes the main classes in the `core` library and how they relate to one another through inheritance ("is-a" relationship) and composition ("has-a" relationship).
...
- **`DecayChannel`**: This class represents a single, specific decay mode (e.g., *p → π⁰ + n*). It is the central coordinator for a decay and **contains collections** of different models:
...
2. **`DecayVectors` (The "How")**: Once the daughter particles are known, a corresponding `DecayVectors` object is used to **generate the decay kinematics**. It answers the question, "What are the momenta/angles of the daughters?"
...
```bash
git clone --recursive https://github.com/dglazier/elSpectro.git
cd elSpectro
```
