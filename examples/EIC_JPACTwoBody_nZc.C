...
TH2F hWt("Wt","Wt",100,0,20,100,0,10);
...
void EIC_JPACTwoBody_nZc(double ebeamE = 5, double pbeamE = 100, double nLumi=1E34, int nDays = 5) {
 ...
 // ---------------------------------------------------------------------------
  ...
  //Zc
  // double minMass = 3.8884;
  //double maxMass = 3.9;
  double minMass = 3.5;
  double maxMass = 4.4;
  mass_distribution(9995,new DistTF1{TF1("hh",Form("TMath::BreitWigner(x,%lf,0.0005)",mZc),minMass,maxMass)});
  auto Z=particle(9995,model(new PhaseSpaceDecay{{jpsi},{211}}));
  Z->SetPdgMass(mZc);

  //create eic electroproduction of X + proton
  auto pGammaStarDecay = JpacTwoBody{&ampZc, {Z},{2112} }; //photo-nucleon system
  ...
  //auto photoprod = DecayModelQ2W{0,&pGammaStarDecay, new TwoBodyFlat()};
  // auto photoprod = DecayModelQ2W{0,&pGammaStarDecay,new TwoBody_stu{1., 1.0, 1.5}};
  ...
  // Initialize HepMC3
  // ---------------------------------------------------------------------------
  writer(new EICSimpleWriter{Form("outTwoBody/jpac_Zc3900_%d_%d.txt",(int)ebeamE,(int)pbeamE)});
  //exit(0);
  ...
  // Generate events
  // ---------------------------------------------------------------------------
  tbd = *dynamic_cast<const TH2D*>(&dynamic_cast<const TwoBodyEnvelope*>(pGammaStarDecay.Parent()->Decayer())->GetDist().GetTH2());
  
  std::cout<<" CEHECK MASS "<<pGammaStarDecay.GetMeson()->P4().M()<<std::endl;
  ...
  TFile *fout = TFile::Open(Form("outTwoBody/jpac6_Zc3900_%d_%d.root",(int)ebeamE,(int)pbeamE), "recreate");
