...
void JpacAmpVectorJpsiPiPi(double ebeamE = 5, double pbeamE = 41, int nEvents = 5e4) {
  ...
  mass_distribution(113,new DistTF1{TF1("hhRho","TMath::BreitWigner(x,0.775,0.151)",0.2,0.9)});
  ...
  auto jpac = new JpacModelQ2W{&sum, {v},{2212}, 0, 1, 2.0};
  ...
  Writer *ascii_io = new WriterAscii(Form("out/vectorJpsiPiPi_elspectro_%d_%d.txt",(int)ebeamE,(int)pbeamE),run);
  ...
  for(int i=0;i<nEvents;i++){
    ...
    particlePtr[3] = std::make_shared<GenParticle>( FourVector( proton->P4().X(), proton->P4().Y(), proton->P4().Z(), proton->P4().E() ), proton->Pdg(), 1 );
    particlePtr[4] = std::make_shared<GenParticle>( FourVector( pionp->P4().X(), pionp->P4().Y(), pionp->P4().Z(), pionp->P4().E() ), pionp->Pdg(), 1 );
    particlePtr[5] = std::make_shared<GenParticle>( FourVector( pionm->P4().X(), pionm->P4().Y(), pionm->P4().Z(), pionm->P4().E() ), pionm->Pdg(), 1 );
    particlePtr[6] = std::make_shared<GenParticle>( FourVector( posJ->P4().X(), posJ->P4().Y(), posJ->P4().Z(), posJ->P4().E() ), posJ->Pdg(), 1 );
    particlePtr[7] = std::make_shared<GenParticle>( FourVector( eleJ->P4().X(), eleJ->P4().Y(), eleJ->P4().Z(), eleJ->P4().E() ), eleJ->Pdg(), 1 );
  ...
  TFile *fout = TFile::Open(Form("out/vectorJpsiPiPi_elspectro_%d_%d_diagnostic.root",(int)ebeamE,(int)pbeamE), "recreate");
